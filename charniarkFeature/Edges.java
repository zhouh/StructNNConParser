...
//
public class Edges extends PTsFeatureClass {
	static class EdgesFeature extends Feature {
		...
		public boolean equals(Object obj) {
			...
			EdgesFeature f = ((EdgesFeature) obj);
	...
	public Edges(boolean binned_length, // include binned length
		...
		identifier_string = "Edges:" + (binned_length ? "1" : "0") + ":"
	...
	public void node_featurecount(FeatureClass fc, Vector<SPTreeNode> preterms,
		...
		EdgesFeature f = new EdgesFeature();
		if(binned_length)
			f.push(symbol_quantize((int)(right - left))); //number of preterminals
		f.push(node.label.cat);							  //category label
		
		for(long i = 1; i <= nleftprec; i++)
			f.push(i <= left ? preterms.get((int)(left - i)).label.cat : endmarker());
		
		for(long i = 0; i < nleftsucc; i++)
			f.push(left + i < nwords ? preterms.get((int)(left + i)).label.cat : endmarker());
		
		for(long i = 1; i <= nrightprec; i++)
			f.push(i <= right ? preterms.get((int)(right - i)).label.cat : endmarker());
		
		for(long i = 0; i < nrightsucc; i++)
			f.push(right + i < nwords ? preterms.get((int)(right + i)).label.cat : endmarker());
	...
	public void read_feature(String fs, long id) {
		...
		EdgesFeature feature = new EdgesFeature();
